...
XLSXReader.prototype.parseXMLWorkbookRelations = function (entry, workbook) {
	var parser = expat.createParser();
	parser.on('startElement', function (name, attrs) {
		if ((name === 'Relationship') && (typeof attrs.Target === 'string')) {
...
XLSXReader.prototype.parseXML = function (cb) {
	...
	function parseSheet() {
		...
		if ((sheet) && (sheet.filename)) {
			lookup = caller.options.dirPrefix + sheet.filename;
			cb({sheet: sheet});
		} else {
			lookup = caller.options.dirPrefix+'worksheets/sheet' + caller.options.sheet_nr + '.xml';
		}
	...
	fs.createReadStream(this.filename)
		.pipe(unzip.Parse())
		.on('entry', function (entry) {
			if (entry.path === caller.options.dirPrefix+'sharedStrings.xml') {
				caller.parseXMLStrings(entry, strings);
			} else if (entry.path === caller.options.dirPrefix+'styles.xml') {
				caller.parseXMLStyles(entry, formatstyles);
			} else if (entry.path === caller.options.dirPrefix+'workbook.xml') {
				caller.parseXMLWorkbook(entry, workbook);
			} else if (entry.path === caller.options.dirPrefix+'_rels/workbook.xml.rels') {
